...
class PointOdysseyDUSt3R(BaseStereoViewDataset):
    def __init__(
        ...
        self.subdirs = []
        self.sequences = []
        self.subdirs.append(os.path.join(self.dataset_location, self.dset))
        ...
        for temp_annot_path in set(self.annotation_paths):
            ...
            temp_seq_name = temp_annot_path.split("/")[-2]
            temp_annots = np.load(temp_annot_path, allow_pickle=True)
    ...
    def _get_views(self, index, resolution, rng):
        ...
        temp_seq_name = annotations_path.split("/")[-2]
        temp_root = f"/root/mast3r/pre_load_po_data{'' if self.dset == 'train' else '_TEST'}/{temp_seq_name}"
        trajs_2d = np.load(f"{temp_root}/trajs_2d.npy", mmap_mode="r")[full_idx].astype(np.float32)
        trajs_3d = np.load(f"{temp_root}/trajs_3d.npy", mmap_mode="r")[full_idx].astype(np.float32)
        vis_n_valids = (np.load(f"{temp_root}/valids.npy", mmap_mode="r")[full_idx] * np.load(f"{temp_root}/visibs.npy", mmap_mode="r")[full_idx])
        vis_n_valids_in_both = (vis_n_valids[0] * vis_n_valids[1])
        trajs_2d = trajs_2d[:, vis_n_valids_in_both]
